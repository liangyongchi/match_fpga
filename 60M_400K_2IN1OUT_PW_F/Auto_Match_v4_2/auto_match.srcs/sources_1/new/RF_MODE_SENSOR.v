...
reg         detect_end ;
wire        off_flag   ;
assign      off_flag = (detect_end_cnt > OFF_NUM);// power nosie can not stop

assign pos_power_fall = power_fall_r0&&(~power_fall_r1);
assign pos_power_rise = power_rise_r0&&(~power_rise_r1);

wire   stage1_sensor_clr;
assign stage1_sensor_clr = (stage1_cnt == detect_pulse_width-32'd1);

wire   power_status_refresh;
assign power_status_refresh = (stage1_cnt == detect_pulse_width-32'd2);

wire   stage1_sensor_result;
assign stage1_sensor_result = (stage1_cnt == detect_pulse_width-32'd2);
...
always@(posedge clk or posedge rst)begin
	  ...
	  else if(power_status_refresh)begin
...
begin 
    if(rst)
	    detect_stage0 <= 1'd0;
	else if(stage0_cnt == match_on_dly-1)
	    detect_stage0 <= 1'd0;
    else if(up_threshold_vld)
	    detect_stage0 <= 1'd1;    
end
...
always@(posedge clk or posedge rst)begin
    if(rst)
        detect_end <= 1'd0;
    else if(stage0_cnt == match_on_dly-1)begin
	    if(off_flag)		
             detect_end <= 1'd1;
	    else 
		     detect_end <= 1'd0;
...
always@(posedge clk or posedge rst)begin
    if(rst)
	   ...
	   if(up_threshold_vld)		
		   ...
		   detect_end_cnt <= detect_end_cnt + 32'd1; 
    end	
	else 
	   detect_end_cnt <= 32'd0;
end
...
begin 
    if(rst)
	    stage1_cnt <= 32'd0;
	     ...
	     if(stage1_cnt >= (detect_pulse_width-32'd1))
	        stage1_cnt <= 32'd0;
		 else
    ...
    if(rst)
	    stage1_neg_cnt <= 32'd0;
         ...
         if(stage1_sensor_clr)
            ...
            stage1_neg_cnt <= stage1_neg_cnt + 32'd1;	  
	end
	else
	    stage1_neg_cnt <= 32'd0; 	
end
